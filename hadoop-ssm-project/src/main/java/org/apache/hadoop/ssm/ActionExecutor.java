...
/**
 * Controls the concurrency of action execution.
 */
final public class ActionExecutor {
  // To control the concurrency of certain type of action
  static private int[] maxConcurrent = new int[ActionType2.values().length];
  static private int[] inFly = new int[ActionType2.values().length];
  static private Semaphore[] semaphores = new Semaphore[ActionType2.values().length];

  static {
    semaphores[ActionType2.DiskBalance.getValue()] = new Semaphore(1);
  }
